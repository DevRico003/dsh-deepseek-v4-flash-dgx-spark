...
**Verifier.** `dsh-verifier-gate` scores with the same model, thinking on, logprobs on, 20-letter scale, the way the llm-as-a-verifier authors score a finished trajectory: once, at the end, at `reasoning_effort: high` (their setting for DeepSeek V4 Flash). A gate at the end of every turn reads the whole turn plus the goal objective and sends findings back when the score is under 0.6; inside a turn the agent calls `verifier_assess` itself before the final answer and `verifier_select` at real forks, at the same effort. There are no mid-turn readings and no edit-count reminders any more; an earlier version had both at `low` and they cost more wall-clock than they found. The calls stream, so a long think never trips a transport timeout; an idle timer catches a dead stream. In the first real run the gate caught the agent declaring a project done while its subagents were still running; later runs caught a test that had been weakened to pass, a lint exit code masked by a pipe, and a TypeScript error introduced in the last edit. Cost at `high` on the Spark pair: 263 to 401 s for the end-of-turn gate over a long turn (a 98k-token prompt), five to eight minutes for a `verifier_assess` the agent calls, `verifier_select` over three candidates fifteen calls; `verifier: backend: reasoningEffort: low` cuts that to a third with verdicts within 0.06 on an A/B here, `none` brings chat sessions to 6 to 10 seconds.
...
**Two harness versions, one home.** DSH Desktop 2.0.1 bundles harness rc.7, the source checkout is 0.1.1; both read `~/.dsh`. The credentials file is where they disagree: 0.1.1 rewrites `.credentials.yaml` into a versioned layout (`version: 1`, `refs:`) that rc.7 rejects on start ("the value for "version" must be a string"), which left Desktop unable to open after one headless run. The headless and web profile patches therefore point the checkout at `~/.dsh/.credentials-0.1.1.yaml` (`credentials: path:`), and Desktop keeps the flat `.credentials.yaml`; `install.sh` creates both from the example. If Desktop ever fails with that message, restore the flat form (`KEY: value` lines) in `.credentials.yaml`.
...
4. Check, after every node: tests, type check, lint, then `ui_snapshot` plus `analyze_image` for anything rendered. Deterministic and cheap.
5. Join: competing candidates go through `verifier_select`.
6. Cycle with a stop: repair, re-check, two rounds at most, then report what is open.
7. Gate and report: one `verifier_assess` over the whole deliverable before the final answer, then the report with every verifier call and its score.

...
`AGENTS.md` in this repo is written for Claude Code, Codex or dsh itself: preflight, config edits, install, check, six smoke tests through the harness, report. Clone the repo, open an agent session in it, and paste:
...
**Desktop.** [DSH Desktop](https://github.com/anywhere-labs/deepseek-harness-desktop) shares `~/.dsh`, so the same settings and plugins apply, and 2.0.2 bundles the same harness as the source checkout (0.1.1-rc.2), so the two agree on session and credentials formats. One rule: do not run `dsh web` and the desktop app at the same time. A web host scans the sessions directory on start, marks in-flight turns of other processes as interrupted, and `dsh-client-auto-continue` then resumes them. Headless sessions therefore live in `~/.dsh/sessions-headless` (`dsh-home/profiles/headless/cordis.patch.yml`).

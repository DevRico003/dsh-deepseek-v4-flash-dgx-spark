...
**Profiles.** `headless` is the one-shot CLI (`bin/dsh --profile headless "task"`). `web` is `bin/dsh web`. `desktop` is DSH Desktop, installed separately from its DMG; it shares `~/.dsh` and bundles the same harness version as the source checkout (2.0.2 carries 0.1.1-rc.2). Run `bin/dsh web` and the desktop app one at a time; a starting web host takes over the other one's in-flight sessions.

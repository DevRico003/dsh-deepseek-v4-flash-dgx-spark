...
5. **Smoke tests through the harness itself.** Run each and read the answer, do not infer it:
   ...
   Done when all four answers are correct.

6. **Report.** List what was installed, the endpoint in use, the four smoke-test outputs verbatim, and anything you changed in `dsh-home/settings.yaml`. Done when the human can reproduce every claim from the report.
